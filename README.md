...
history readable — only the playbooks that actually enriched something show up. To set that up in
the Sentinel Portal:
...
| File hash | Entity → File hash → Value (or Algorithm) | Contains | *(blank)* | `Enrich-FileHash-IncidentComment` |
| Mail message | Entity → Mail message → Network message Id | Contains | *(blank)* | `Enrich-Email-IncidentComment` |
