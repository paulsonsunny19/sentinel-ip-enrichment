...
def approval_gate_actions(prefix, task_title_expr, task_description, timeout_iso="PT30M", poll_interval_seconds=30):
    """Sentinel-incident-task-based approval gate ("Option C" -- see
    README-RESPONSE.md's approval-gate section). Creates an incident task,
    then polls (Until + Delay) for an analyst to mark it Completed from
    the incident's own Tasks tab in the Sentinel Portal, before the
    caller's actual write action is allowed to run. Stays entirely on the
    same ManagedServiceIdentity/ARM_AUTH auth every other ARM call in this
    repo uses -- Microsoft Sentinel Responder RBAC (already granted
    everywhere) is sufficient, no delegated Teams/Outlook connection.

    Returns the create+poll actions as a dict to merge into the caller's
    own actions dict. The caller is responsible for wrapping its actual
    destructive HTTP call in a Condition checking
    @equals(variables('{prefix}ApprovalStatus'), 'Completed') -- this
    function only gates the *wait*, not the action itself, since what
    happens on approval is different for every call site.

    prefix: unique per call site in the workflow (e.g. "ResetPassword"),
    used to namespace action/variable names so this can be used more than
    once in the same workflow without collisions.
    task_title_expr: raw WDL expression (no leading @) for the task title.
    task_description: plain string (no interpolation) for the task body.
    timeout_iso / poll_interval_seconds: how long to wait and how often to
    re-check. Not exposed as deployment parameters (kept simple/hardcoded
    for now) -- ask if you want these tunable per deployment.

    Uses the Sentinel Incident Tasks REST API: PUT/GET against
    {incident's own ARM resource ID}/tasks/{taskId}?api-version=2024-03-01.
    Built from this session's own research into that (Preview) API; its
    docs weren't independently reachable to re-verify live while writing
    this, so -- same as every other REST integration in this repo --
    confirm the exact request/response shape against a real incident
    before relying on it, and report back anything that doesn't match so
    it can be corrected.
    """
    compose_id = f"Compose_{prefix}TaskId"
    create = f"HTTP_{prefix}CreateTask"
    until = f"Until_{prefix}Approval"
    delay = f"Delay_{prefix}Approval"
    check = f"HTTP_{prefix}CheckTask"
    set_status = f"Set_{prefix}ApprovalStatus"
    status_var = f"{prefix}ApprovalStatus"

    task_url_expr = (
        f"@{{concat(triggerBody()?['object']?['id'], '/tasks/', outputs('{compose_id}'), "
        "'?api-version=2024-03-01')}"
    )

    return {
        compose_id: {"runAfter": {}, "type": "Compose", "inputs": "@guid()"},
        create: {
            **http_call(
                task_url_expr, method="PUT", auth=ARM_AUTH,
                body={
                    "properties": {
                        "title": f"@{{{task_title_expr}}}",
                        "description": task_description,
                        "status": "New",
                    },
                },
            ),
            "runAfter": after(compose_id),
        },
        until: {
            "type": "Until",
            "expression": f"@equals(variables('{status_var}'), 'Completed')",
            "limit": {"count": 60, "timeout": timeout_iso},
            "actions": {
                delay: {
                    "type": "Wait", "runAfter": {},
                    "inputs": {"interval": {"count": poll_interval_seconds, "unit": "Second"}},
                },
                check: {
                    **http_call(task_url_expr, method="GET", auth=ARM_AUTH),
                    "runAfter": after(delay),
                },
                set_status: {
                    "runAfter": after(check, states=("Succeeded", "Failed", "Skipped", "TimedOut")),
                    "type": "SetVariable",
                    "inputs": {
                        "name": status_var,
                        "value": (
                            f"@if(equals(outputs('{check}')?['statusCode'], 200), "
                            f"coalesce(outputs('{check}')?['body']?['properties']?['status'], variables('{status_var}')), "
                            f"variables('{status_var}'))"
                        ),
                    },
                },
            },
            "runAfter": after(create, states=("Succeeded", "Failed", "Skipped", "TimedOut")),
        },
    }
