...
## Entity-trigger variants (Preview): run from a single entity instead of the incident

Five playbooks also ship an **entity-trigger** variant, alongside (not replacing) their
incident-trigger original:

| Entity-trigger playbook | Default name | Entity | Based on |
|---|---|---|---|
| Account: revoke sessions | `ErgoSOC-AU-Account-RevokeSessions-EntityTrigger` | Account | `azuredeploy-response-account-revoke-sessions.json` |
| Account: reset password | `ErgoSOC-AU-Account-ResetPassword-EntityTrigger` | Account | `azuredeploy-response-account-reset-password.json` |
| Account: revoke app consent | `ErgoSOC-AU-Account-RevokeAppConsent-EntityTrigger` | Account | `azuredeploy-response-account-revoke-consent.json` |
| Account: disable + confirm compromised | `ErgoSOC-AU-Account-DisableAndConfirmCompromised-EntityTrigger` | Account | `azuredeploy-response-account-disable.json` |
| IP: block indicator | `ErgoSOC-AU-Indicator-Block-IP-EntityTrigger` | IP | IP half of `azuredeploy-response-indicator-block.json` |

Generated by `build_response_account_revoke_sessions_entity.py`,
`build_response_account_reset_password_entity.py`,
`build_response_account_revoke_consent_entity.py`, `build_response_account_disable_entity.py`,
and `build_response_indicator_block_ip_entity.py` — files ending `..._entity.py` /
`azuredeploy-..._entity.json` throughout this repo are the entity-trigger variants.

**Microsoft Sentinel's "entity" trigger** (still Preview) fires on a single entity — select it
inside an incident's overview blade (or from Hunting, standalone), then Actions → Run playbook —
instead of an incident and everything related to it. Two real platform differences from the
incident trigger, not choices made here:

- **It currently supports only Account and IP entities.** That's why only these five playbooks
  have an entity-trigger variant; the URL half of Indicator Block, FileHash Block, Device
  Isolate/Scan, and Email Block don't, because Host/URL/FileHash/Mail message entities aren't
  supported by this trigger type yet.
- **It cannot be attached to a Sentinel automation rule** (a different trigger schema than
  automation rules expect) — irrelevant here anyway, since none of this repo's response playbooks
  are wired to one (see above).

Because it fires on exactly one entity, there's no "Entities - Get..." connector action and no
Foreach loop — the entity's own fields are read directly off the trigger body. The account-entity
field-resolution logic (AadUserId/UPN/display name, falling back to a Graph lookup when needed) is
shared by all four Account entity-trigger playbooks via one helper,
`response_common.entity_account_resolve_actions()`, rather than copied four times — that logic
shipped two real bugs the first time it was hand-duplicated (a malformed nested `if()`, and a
null-vs-empty-string comparison bug on the Incident ARM ID field), so it's fixed once, centrally,
now.

**Confidence varies by playbook.** The Account trigger shape (trigger path, direct
`triggerBody()?['Entity']?['properties']?[...]` field reads, the `IncidentArmID` field and its
casing) is confirmed against a complete, working reference sample and against real deployment
errors that got fixed. **The IP entity-trigger playbook is not independently verified the same
way** — its entity-type string and address field name are best-confidence guesses by analogy with
the confirmed Account pattern (see that file's own docstring for the reasoning). Test it and
report back what actually happens; if either guess is wrong, it's a one-line fix.

`scripts/check-wdl-arity.py` checks every generated template's `if()`/`equals()`/`not()` calls for
correct argument count — run it (alongside the existing `scripts/validate-logic-app-json.py`,
which checks action reachability, not expression syntax) after touching any hand-built WDL
expression string.
